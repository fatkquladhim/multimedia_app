...
try {
    ...
    // Ambil data profil pengguna yang login
    $user_id = $_SESSION['user_id'];
    $stmt_profile = $conn->prepare("SELECT username, nama_lengkap, foto FROM profile WHERE id = ?");
    ...
    <div class="min-h-screen flex">
        ...
        <div id="mainContentArea" class="flex-1 flex flex-col main-content-area">
                        ...
                        <div class="relative">
                            <button id="profileDropdownToggle" class="flex items-center space-x-2 focus:outline-none">
                                <div class="w-8 h-8 md:w-10 md:h-10 bg-light-blue-300 rounded-full overflow-hidden">
                                    <?php if (!empty($user_profile['foto'])): ?>
                                        <img src="../uploads/profiles/<?php echo htmlspecialchars($user_profile['foto']); ?>" alt="Profile" class="w-full h-full object-cover">
                                    <?php else: ?>
                                        <img src="https://via.placeholder.com/40x40" alt="Profile" class="w-full h-full object-cover">
                                    <?php endif; ?>
                                </div>
                                <span class="hidden md:block text-gray-800 dark:text-white font-medium"><?php echo htmlspecialchars($user_profile['nama_lengkap'] ?? $user_profile['username']); ?></span>
                                <i class="fas fa-chevron-down text-gray-600 dark:text-gray-300 text-xs"></i>
                            </button>

                            <div id="profileDropdownMenu" class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-700 rounded-md shadow-lg py-1 z-50 hidden">
                                <a href="profile/profile_view.php" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-600">
                                    <i class="fas fa-user-circle mr-2"></i>Profil Saya
                                </a>
                                <a href="../auth/logout.php" class="block px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-slate-600">
            ...
            <main class="flex-1 p-4 md:p-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 hover-scale animate-fade-in">
