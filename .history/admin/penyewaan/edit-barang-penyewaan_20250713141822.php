...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $conn->prepare("SELECT sb.*, a.nama as nama_anggota, al.nama_alat as nama_alat 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($stok['jumlah'] >= $jumlah_baru) {
            // Update penyewaan
            $stmt = $conn->prepare("UPDATE penyewaan_barang SET id_alat = ?, jumlah = ?, biaya = ?, tanggal_kembali = ? WHERE id = ?");
            $stmt->bind_param("iidsi", $id_alat_baru, $jumlah_baru, $biaya, $tanggal_kembali, $id);
            $stmt->execute();
...
<body>
    <div class="container">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                <label>Biaya:</label>
                <input type="number" name="biaya" value="<?php echo $penyewaan['biaya']; ?>" min="0" step="1000" required>
            </div>
