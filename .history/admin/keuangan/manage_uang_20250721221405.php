...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_data'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO keuangan (tanggal, keterangan, pemasukan, pengeluaran, saldo)
                       VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("ssiii", $tanggal, $keterangan, $pemasukan, $pengeluaran, $saldo_akhir);
    $stmt->execute();
                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td class="text-success">Rp <?= number_format($row['pemasukan']) ?></td>
                                <td class="text-danger">Rp <?= number_format($row['pengeluaran']) ?></td>
                                <td class="font-weight-bold">Rp <?= number_format($row['saldo']) ?></td>
                                <td>
                    ...
                    <tfoot>
                        <tr class="font-weight-bold">
                            ...
                            <td class="text-success">Rp <?= number_format($total_pemasukan) ?></td>
                            <td class="text-danger">Rp <?= number_format($total_pengeluaran) ?></td>
                            <td>Rp <?= number_format($total_pemasukan - $total_pengeluaran) ?></td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
