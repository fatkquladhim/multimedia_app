...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_data'])) {
    ...
    $stmt = $conn->prepare("UPDATE keuangan SET 
                          ...
                          saldo = ?
                          WHERE id = ?");
    $stmt->bind_param("siissii", $tanggal, $pemasukan, $pengeluaran, $keterangan_pemasukan, $keterangan_pengeluaran, $saldo_akhir, $id);
    $stmt->execute();
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" name="keterangan_pemasukan" class="form-control" value="<?= htmlspecialchars($data['keterangan_pemasukan']) ?>">
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label>Keterangan Pengeluaran:</label>
                    <input type="text" name="keterangan_pengeluaran" class="form-control" value="<?= htmlspecialchars($data['keterangan_pengeluaran']) ?>">
                </div>
            </div>
        </div>
