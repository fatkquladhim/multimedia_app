...
try {
    ...
    $conn->set_charset("utf8mb4"); // Set charset untuk menghindari masalah encoding
    ...
    // Menggunakan prepared statement untuk keamanan, meskipun input tidak langsung dari user
    ...
    // Menambahkan a.id ke SELECT statement
    $stmt_izin_malam = $conn->prepare("SELECT a.id, a.nama, im.tanggal, im.jam_izin, im.jam_selesai_izin 
                                       FROM izin_malam im 
    ...
    $stmt_top_anggota = $conn->prepare("SELECT id, nama FROM anggota ORDER BY id DESC LIMIT 4");
    $stmt_top_anggota->execute();
    $top_anggota = $stmt_top_anggota->get_result();

} catch (mysqli_sql_exception $e) {
    // Catat error ke log (misalnya, error_log($e->getMessage());)
                        ...
                        <?php while($row = $tugas_reviews->fetch_assoc()): ?>
                            <div class="bg-white dark:bg-slate-800 rounded-xl p-4 md:p-6 shadow-sm hover-scale animate-fade-in">
                                ...
                                <button class="w-full px-4 py-2 bg-light-blue-600 hover:bg-light-blue-700 text-white rounded-lg text-sm font-medium transition-colors">
                                    <a href="beri tugas/tugas_user_review.php?id_tugas=<?php echo htmlspecialchars($row['id']); ?>" class="block">Nilai Tugas</a>
                                </button>
                            </div>
                        <?php endwhile; ?>
                    <?php else: ?>
                                    ...
                                    <div class="flex items-center justify-between p-4 bg-light-blue-50 dark:bg-light-blue-900 rounded-xl hover-scale">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-600 dark:text-gray-400">Member #<?php echo htmlspecialchars($row['id']); ?></p>
                                            </div>
                                        </div>
                            ...
                            <?php if ($top_anggota->num_rows > 0): ?>
                                <?php while($row = $top_anggota->fetch_assoc()): ?>
                                    <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-700 rounded-xl hover-scale">
                                        <div class="flex items-center space-x-3">
                                            <div class="w-10 h-10 bg-light-blue-100 dark:bg-light-blue-800 rounded-lg flex items-center justify-center">
                                                <i class="fas fa-user text-light-blue-600 dark:text-light-blue-400"></i>
                                            </div>
                                            <div>
                                                <h4 class="font-semibold text-gray-800 dark:text-white"><?php echo htmlspecialchars($row['nama']); ?></h4>
