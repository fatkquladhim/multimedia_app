<?php
// sidebar.php
// Pastikan session sudah dimulai di halaman yang menyertakan ini
// session_start(); // Jangan panggil di sini jika sudah dipanggil di halaman utama

// Ambil data profil pengguna yang login untuk sidebar
// Asumsi $conn sudah tersedia dari halaman utama
if (isset($_SESSION['user_id'])) {
    require_once dirname(__DIR__) . '/includes/db_config.php'; // Adjust path as needed
    $conn_sidebar = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
    $stmt_sidebar = $conn_sidebar->prepare("SELECT username, nama_lengkap, foto FROM profile WHERE id = ?");
    $stmt_sidebar->bind_param("i", $_SESSION['user_id']);
    $stmt_sidebar->execute();
    $result_sidebar = $stmt_sidebar->get_result();
    $user_profile_sidebar = $result_sidebar->fetch_assoc();
    $stmt_sidebar->close();
    $conn_sidebar->close();
}
?>
