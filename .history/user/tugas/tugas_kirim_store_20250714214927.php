...
// Check if task exists and belongs to user, and if it's not already submitted
...
if ($result_submission->num_rows > 0) {
    header('Location: tugas_user.php?status=info&message=Anda sudah mengirimkan jawaban untuk tugas ini.');
...
if (isset($_FILES['file_jawaban']) && $_FILES['file_jawaban']['error'] === UPLOAD_ERR_OK) {
    ...
    $file_type = $_FILES['file_jawaban']['type'];
...
if ($stmt_insert_jawaban->execute()) {
    ...
    $stmt_update_tugas = $conn->prepare('UPDATE tugas SET status = "selesai" WHERE id = ?');
    $stmt_update_tugas->bind_param('i', $id_tugas);
    $stmt_update_tugas->execute();
    $stmt_update_tugas->close();

    header('Location: tugas_user.php?status=success&message=Jawaban berhasil dikirim.');
} else {
    ...
    header('Location: tugas_user.php?status=error&message=Gagal mengirim jawaban. Silakan coba lagi.');
}
