...
// Jika tidak ada tugas pending/selesai, ambil tugas terakhir yang sudah dinilai atau riwayat tugas
$last_task_result = null;
if ($result->num_rows == 0) {
    $stmt_last_task = $conn->prepare('
        SELECT
            t.id,
            t.judul,
            t.deskripsi,
        ...
        FROM tugas t
        LEFT JOIN tugas_jawaban tj ON t.id = tj.id_tugas AND tj.id_user = ?
        WHERE t.id_penerima_tugas = ?
        ORDER BY t.deadline DESC, t.id DESC
        LIMIT 1
    ');
    $stmt_last_task->bind_param('ii', $id_user, $id_user);
    $stmt_last_task->execute();
    $last_task_result = $stmt_last_task->get_result();
}
?>
...
<head>
    ...
    <style>
        ...
        .sidebar {
            ...
            overflow-y: auto; /* Tambahkan ini untuk scroll jika konten sidebar melebihi tinggi */
            height: 100vh; /* Pastikan sidebar mengambil tinggi penuh viewport */
            position: sticky; /* Opsional: membuat sidebar tetap di tempat saat scroll halaman utama */
            top: 0; /* Opsional: jika menggunakan position: sticky */
        }
        .sidebar-text {
            transition: opacity 0.3s ease-in-out, margin-left 0.3s ease-in-out, width 0.3s ease-in-out; /* Tambahkan width untuk transisi yang lebih halus */
        }
        .sidebar-nav-item {
            justify-content: flex-start; /* Default alignment for expanded state */
            white-space: nowrap; /* Mencegah teks melipat saat sidebar menyempit */
        }
        ...
        /* Hapus atau sesuaikan jika Anda memiliki elemen logo teks terpisah */
        ...
        /* Hapus atau sesuaikan jika Anda memiliki tombol create terpisah */
...
<body>
    ...
    ?>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $tasks_to_display[] = $row;
                }
            } elseif ($last_task_result && $last_task_result->num_rows > 0) {
                $tasks_to_display[] = $last_task_result->fetch_assoc();
                $message_no_tasks = '<div class="col-span-full text-center text-gray-600 py-4">Tidak ada tugas baru. Ini adalah tugas terakhir Anda:</div>';
            } else {
                $message_no_tasks = '<div class="col-span-full text-center text-gray-600 py-8">Tidak ada tugas yang tersedia saat ini.</div>';
            }

            if (!empty($message_no_tasks)) {
                echo $message_no_tasks;
            }

            foreach ($tasks_to_display as $row):
                    // Determine card color based on task status or type (optional)
                    $card_bg_color = 'bg-purple-50';
                    $icon_bg_color = 'bg-purple-100';
                    $icon_color = 'text-purple-600';
            ...
            ?>
                    <div class="<?= $card_bg_color ?> rounded-2xl p-6 relative overflow-hidden">
                        ...
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-gray-700">Status:
                                <?php
                                    if ($row['jawaban_id'] && $row['status'] == 'selesai') {
                                        ...
                                        echo 'Sudah Dinilai (Nilai: ' . htmlspecialchars($row['nilai']) . ')'; // Admin has graded
                                    } else {
                                        echo htmlspecialchars($row['status']); // Pending
                                    }
                                ?>
                            ...
                            <span class="text-sm font-bold <?= $action_color ?>">
                                <?php if (!$row['jawaban_id'] && $row['status'] != 'diperiksa'): // If no answer submitted yet and not graded ?>
                                    <a href="./tugas/tugas_kerjakan.php?id=<?php echo $row['id']; ?>">Kerjakan</a>
                                <?php elseif ($row['jawaban_id'] && $row['status'] == 'selesai'): // If answer already submitted and waiting for grading ?>
                                    Menunggu Penilaian
                                <?php elseif ($row['jawaban_id'] && $row['status'] == 'diperiksa'): // If graded ?>
                                    Sudah Dinilai
                                <?php else: ?>
                                    Sudah dikerjakan
                                <?php endif; ?>
    ...
    <script>
        ...
        sidebarToggle.addEventListener('click', () => {
            if (isSidebarOpen) {
                ...
                // sidebarLogoText.classList.add('opacity-0', 'pointer-events-none'); // Hapus baris ini jika ingin logo tetap terlihat
                ...
                // sidebarCreateButton.classList.remove('space-x-2'); // Hapus atau sesuaikan jika ada tombol create
                ...
                // sidebarLogoText.classList.remove('opacity-0', 'pointer-events-none'); // Hapus baris ini jika ingin logo tetap terlihat
                ...
                // sidebarCreateButton.classList.remove('justify-center'); // Hapus atau sesuaikan jika ada tombol create
        ...
        // Tambahkan ini untuk memastikan sidebar bisa di-scroll jika kontennya panjang
        // Ini lebih baik ditangani dengan CSS `overflow-y: auto;` pada elemen sidebar
