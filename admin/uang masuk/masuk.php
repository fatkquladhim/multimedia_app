...
if (isset($_GET['hapus']) && is_numeric($_GET['hapus'])) {
    $id = $_GET['hapus'];
    $result = $conn->query("SELECT id FROM uang_masuk WHERE id=$id");
    if ($result->num_rows > 0) {
        $conn->query("DELETE FROM uang_masuk WHERE id=$id");
        header("Location: masuk.php?status=success&message=Data dihapus");
    } else {
        header("Location: masuk.php?status=error&message=ID tidak valid atau data otomatis tidak dapat dihapus");
    }
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        header("Location: masuk.php?status=error&message=ID tidak valid atau data otomatis tidak dapat diedit");
    }
    exit;
}
...
$q_auto = "SELECT NULL as id, p.tanggal_kembali as tanggal, p.biaya as jumlah, CONCAT('Otomatis dari penyewaan ID: ', p.id, ' (', IFNULL(p.nama_penyewa, '-'), ')') as keterangan, p.status FROM penyewaan_barang p WHERE p.status='dikembalikan' AND p.biaya > 0";
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($row['keterangan']); ?> (Barang Manual)</td>
                    <td>
                        <a href="edit_pemasukan.php?id=<?php echo $row['id']; ?>">Edit</a> |
                ...
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($row['tanggal']); ?></td>
                    <td>Rp <?php echo number_format($row['jumlah'], 0, ',', '.'); ?></td>
                    <td><?php echo htmlspecialchars($row['keterangan']); ?> (Barang Otomatis)</td>
                    <td>-</td>
                </tr>
                <?php endwhile; ?>
            </tbody>
